...
[EnsemblPlants release-48](ftp://ftp.ensemblgenomes.org/pub/plants/release-48/fasta/)
can used.
...
Here, as an example, the GFF3 file for the species *Arabidopsis thaliana* is
obtained from the FTP server from
[Ensembl Plants](https://plants.ensembl.org/index.html).

__Note:__ The GFF3 and GTF files can be very large and it might take some time
...
g.min_max <- plot_kaks(ath_aly_ncbi_kaks,
    ka.min = 0,
    ka.max = 1,
    ks.min = 0,
    ks.max = 1)
...
HOMSAP.cds.longest.genepos <- cds2genepos(HOMSAP.cds.longest, 
    source = "ENSEMBL")
## get gene position for P. troglodytes longest isoforms
PANTRO.cds.longest.genepos <- cds2genepos(PANTRO.cds.longest,
    source = "ENSEMBL")
