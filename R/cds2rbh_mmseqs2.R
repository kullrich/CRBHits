cds2rbh_mmseqs2 <- function(cds1, cds2,
    mmseqs2path="/Users/ullrich/anaconda3/envs/mmseqs2/bin/",
    ...
    fitSpline <- function(alnlength, evalue, fit.type, fit.varweight, fit.min){
        ...
        if(!dir.exists(mmseqs2path)){
            stop("Error: mmseqs2 PATH does not exist. Please specify correct PATH
                and/or look into package installation prerequisites.")
        }
        if(!file.exists(paste0(mmseqs2path, "mmseqs"))){
            stop("Error: mmseqs2 binary does not exist. Please specify correct
        ...
        aa2_aa1_lastout <- tempfile("aa2_aa1_mmseqs2out_", outpath)
        aa1_aa2_lastout <- tempfile("aa1_aa2_mmseqs2out_", outpath)
        if(longest.isoform){
        ...
        system2(command=paste0(mmseqs2path, "mmseqs"),
            args = c("easy-search", aa1file, aa2file, aa1_aa2_lastout, outpath, "--threads", threads, "-s", mmseqs2sensitivity, "--format-output", "query,target,fident,alnlen,mismatch,gapopen,qstart,qend,tstart,tend,evalue,bits,qlen,tlen,raw"))
        system2(command=paste0(mmseqs2path, "mmseqs"),
            args = c("easy-search", aa2file, aa1file, aa2_aa1_lastout, outpath, "--threads", threads, "-s", mmseqs2sensitivity, "--format-output", "query,target,fident,alnlen,mismatch,gapopen,qstart,qend,tstart,tend,evalue,bits,qlen,tlen,raw"))
