...
cds2codonaln <- function(cds1, cds2, type = "global", substitutionMatrix = "BLOSUM62", gapOpening = 10, gapExtension = 0.5, remove.gaps = FALSE){
  if(class(cds1) == "DNAString"){
    x.aa <- cds2aa(Biostrings::DNAStringSet(cds1))[[1]]
    x.name <- "cds1"
  }
  if(class(cds2) == "DNAString"){
    y.aa <- cds2aa(Biostrings::DNAStringSet(cds2))[[1]]
    y.name <- "cds2"
  }
  if(class(cds1) == "DNAStringSet" && length(cds1) != 1){
    stop("Error: cds1 needs to only contain one sequence")
  }
  if(class(cds1) == "DNAStringSet" && length(cds1) == 1){
    x.aa <- cds2aa(cds1)[[1]]
    x.name <- names(cds1)
  ...
  if(class(cds2) == "DNAStringSet" && length(cds2) != 1){
    stop("Error: cds2 needs to only contain one sequence")
  }
  if(class(cds2) == "DNAStringSet" && length(cds2) == 1){
    y.aa <- cds2aa(cds2)[[1]]
    y.name <- names(cds2)
  ...
  #xy.aln <- Biostrings:::.makePostalignedSeqs(Biostrings::pairwiseAlignment(x.aa, y.aa, type = type, substitutionMatrix = substitutionMatrix, gapOpening = gapOpening, gapExtension = gapExtension))[[1L]]
  xy.aln <- makePostalignedSeqs(Biostrings::pairwiseAlignment(x.aa, y.aa, type = type, substitutionMatrix = substitutionMatrix, gapOpening = gapOpening, gapExtension = gapExtension))[[1L]]
  ...
  x.cds <- Biostrings::subseq(cds1, (xy.aln@ranges@start[1]*3)-2, (xy.aln@ranges@width[1]-sum(xy.aln.pattern.gap.len))*3)
  y.cds <- Biostrings::subseq(cds2, (xy.aln@ranges@start[2]*3)-2, (xy.aln@ranges@width[2]-sum(xy.aln.subject.gap.len))*3)
  ...
  for(i in seq(from=1, to=(length(xy.aln.pattern.gap.pos)))){
    ...
    if(i == 1){
      if(cur.gap.pos == 1){
        #add gaps
        gaps <- Biostrings::DNAString(paste(rep("---", cur.gap.len), collapse=""))
        tmp.x <- c(tmp.x, gaps)
      }
      if(cur.gap.pos != 1){
        ...
        tmp.x <- c(tmp.x, Biostrings::subseq(x.cds, tmp.x.cur.start, tmp.x.cur.end))
        #add gaps
        gaps <- Biostrings::DNAString(paste(rep("---", cur.gap.len), collapse=""))
        tmp.x <- c(tmp.x, gaps)
      }
    ...
    if(i != 1){
      ...
      tmp.x.cur.end <- tmp.x.cur.end+(cur.gap.pos-xy.aln.pattern.gap.pos[i-1]-xy.aln.pattern.gap.len[i-1])*3
      tmp.x<-c(tmp.x, Biostrings::subseq(x.cds,tmp.x.cur.start,tmp.x.cur.end))
      #add gaps
      gaps <- Biostrings::DNAString(paste(rep("---",cur.gap.len), collapse=""))
      tmp.x <- c(tmp.x, gaps)
  ...
  for(i in seq(from=1, to=(length(xy.aln.subject.gap.pos)))){
    ...
    if(i == 1){
      ...
      if(cur.gap.pos != 1){
        ...
        tmp.y <- c(tmp.y, Biostrings::subseq(y.cds, tmp.y.cur.start, tmp.y.cur.end))
        #add gaps
        gaps <- Biostrings::DNAString(paste(rep("---", cur.gap.len), collapse=""))
        tmp.y <- c(tmp.y, gaps)
      }
    ...
    if(i != 1){
      ...
      tmp.y.cur.end <- tmp.y.cur.end+(cur.gap.pos-xy.aln.subject.gap.pos[i-1]-xy.aln.subject.gap.len[i-1])*3
      tmp.y <- c(tmp.y, Biostrings::subseq(y.cds, tmp.y.cur.start, tmp.y.cur.end))
      #add gaps
      gaps <- Biostrings::DNAString(paste(rep("---", cur.gap.len), collapse=""))
      tmp.y <- c(tmp.y, gaps)
