#' @title cds2genepos
#' @name cds2genepos
#' @description This function extracts the gene position from either NCBI or ENSEMBL CDS input.
#' @param cds \code{DNAStringSet} [mandatory]
#' @param source source indicating either NCBI or ENSEMBL [default: NCBI]
#' @param keep.names vector indicating gene ids to be kept before chromosomal position assignment [default: NULL]
#' @return \code{matrix}
#' 1: $gene.seq.id\cr
#' 2: $gene.chr\cr
#' 3: $gene.start\cr
...
cds2genepos <- function(cds, source = "NCBI", keep.names = NULL){
  ...
  if(source == "NCBI"){
    ...
    gene.start <- as.numeric(gsub("<", "", unlist(lapply(strsplit(gene.location, "\\.\\."),
  ...
  if(!is.null(keep.names)){
    gene.pos.df <- gene.pos.df[gene.pos.df$gene.seq.id %in% keep.names , , drop=FALSE]
  }
