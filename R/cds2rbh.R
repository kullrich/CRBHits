...
cds2rbh <- function(cds1, cds2,
  ...
  fitSpline <- function(alnlength, evalue, fit.type, fit.varweight, fit.min){
    ...
    for(i in seq(from = 1, to = x.max)){
      ...
      smin <- i - s
      smax <- i + s
      s.idx <- BiocGenerics::which(x[, 1] >= smin & x[, 1] <= smax)
  ...
  rbh1 <- aa1_aa2.dedup[BiocGenerics::which(aa1_aa2.dedup.idx %in% aa2_aa1.dedup.idx), , drop = FALSE]
  rbh2 <- aa2_aa1.dedup[BiocGenerics::which(aa2_aa1.dedup.idx %in% aa1_aa2.dedup.idx), , drop = FALSE]
  if(selfblast){
  ...
  if(crbh){
    ...
    rbh1.sec <- aa1_aa2.red.dedup[BiocGenerics::which(aa1_aa2.red.dedup.idx %in% aa2_aa1.red.dedup.idx), , drop = FALSE]
    rbh2.sec <- aa2_aa1.red.dedup[BiocGenerics::which(aa2_aa1.red.dedup.idx %in% aa1_aa2.red.dedup.idx), , drop = FALSE]
    if(selfblast){
    ...
    single1 <- aa1_aa2.red.dedup[BiocGenerics::which(!aa1_aa2.red.dedup.idx %in% aa2_aa1.red.dedup.idx), , drop = FALSE]
    single2 <- aa2_aa1.red.dedup[BiocGenerics::which(!aa2_aa1.red.dedup.idx %in% aa1_aa2.red.dedup.idx), , drop = FALSE]
