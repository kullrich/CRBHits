...
#' (CRBHit) pairs from two AA fasta files.
...
aafile2rbhplus <- function(aafile1, aafile2,
    dbfile1=NULL,
    dbfile2=NULL,
    searchtool="last",
  ...
  if(searchtool=="last"){
    colnames(aa1_aa2)[16:17] <- colnames(aa2_aa1)[16:17] <- c("q_seq", "s_seq")
  }
  if(searchtool=="mmseqs2"){
  ...
  if(remove){
    ...
    if(searchtool=="last"){
        system2(command="rm", args = aa2_aa1_mafout)
        system2(command="rm", args = aa1_aa2_mafout)
    }
  }
  if(remove.db){
  ...
  if(crbh){
    ...
    if(!keepSingleDirection){
      if(dim(rbh1.sec)[1]==0){
        ...
        colnames(crbh1)[dim(crbh1)[2]] <- "rbh_class"
      } else{
        crbh1 <- data.frame(Map(c ,cbind(rbh1, "rbh"),
                                cbind(rbh1.sec[, seq_len(dim(rbh1.sec)[2]-1)], "sec")))
        colnames(crbh1)[dim(crbh1)[2]] <- "rbh_class"
      }
      if(dim(rbh2.sec)[1]==0){
        crbh2 <- data.frame(Map(c ,cbind(rbh2, "rbh")))
        colnames(crbh2)[dim(crbh2)[2]] <- "rbh_class"
      } else{
        crbh2 <- data.frame(Map(c ,cbind(rbh2, "rbh"),
                                cbind(rbh2.sec[, seq_len(dim(rbh2.sec)[2]-1)], "sec")))
        colnames(crbh2)[dim(crbh2)[2]] <- "rbh_class"
      }
      crbh <- crbh1[, c("query_id", "subject_id", "rbh_class")]
    ...
    if(keepSingleDirection){
      ...
      if(dim(rbh1.sec)[1]==0 & dim(single1)[1]!=0){
        crbh1 <- data.frame(Map(c, cbind(rbh1, "rbh"),
                                cbind(single1[, seq_len(dim(single1)[2]-1)], "single")))
        colnames(crbh1)[dim(crbh1)[2]] <- "rbh_class"
      } else{
        crbh1 <- data.frame(Map(c, cbind(rbh1, "rbh"),
                                cbind(rbh1.sec[, seq_len(dim(rbh1.sec)[2]-1)], "sec"),
      ...
      if(dim(rbh2.sec)[1]==0 & dim(single2)[1]!=0){
        crbh2 <- data.frame(Map(c, cbind(rbh2, "rbh"),
                                cbind(single2[, seq_len(dim(single2)[2]-1)], "single")))
        colnames(crbh2)[dim(crbh2)[2]] <- "rbh_class"
      } else{
        crbh2 <- data.frame(Map(c, cbind(rbh2, "rbh"),
                                cbind(rbh2.sec[, seq_len(dim(rbh2.sec)[2]-1)], "sec"),
      ...
      crbh <- data.frame(Map(c, crbh1[, c("query_id", "subject_id", "rbh_class")],
                             single1[, c("query_id", "subject_id", "rbh_class")], single2[, c("query_id", "subject_id", "rbh_class")]))
      colnames(crbh) <- c("aa1", "aa2", "rbh_class")
      out <- list(crbh, crbh1, crbh2, rbh1_rbh2_fit)
      names(out) <- c("crbh.pairs", "crbh1", "crbh2", "rbh1_rbh2_fit")
      attr(out, "CRBHits.class") <- "crbh"
