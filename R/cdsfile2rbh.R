#' @title cdsfile2rbh
#' @name cdsfile2rbh
#' @description This function calculates (conditional-)reciprocal best hit pair matrix from two cds fasta file's.
...
#' @param filter specify the filters to be applied on hit pairs e.g.: c("filter.eval", "filter.rost1999") [default: NULL]
...
cdsfile2rbh <- function(cdsfile1, cdsfile2,
  ...
  #apply filter on hit pairs
  for(f in filter){
    f_ <- get(f)
    aa1_aa2 <- aa1_aa2[apply(aa1_aa2, 1, function(x) f_(x)), , drop = FALSE]
    aa2_aa1 <- aa2_aa1[apply(aa2_aa1, 1, function(x) f_(x)), , drop = FALSE]
  }
