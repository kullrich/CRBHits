#' @title make_vignette
#' @name make_vignette
#' @description This function tries to build the prerequisite last-1595,
...
make_vignette <- function(){
    ...
    system2(command="unzip", args=c("-o",
        paste0(CRBHits_root, "/extdata/last-1595.zip"), "-d", LastTempDir))
    system2(command="cd", args=c(paste0(LastTempDir, "/last-1595/;"), "make"))
    ...
    system2(command="cd", args=c(paste0(DAGchainerTempDir, "/dagchainer/;"),
        "make"))
    return(c(
        paste0(LastTempDir, "/last-1595/bin/"),
        #paste0(KaKsCalcTempDir, "/KaKs_Calculator2.0_src/src/"),
