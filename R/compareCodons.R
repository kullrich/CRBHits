#' @title compareCodons
#' @name compareCodons
#' @description This function compares two codons and returns the number of syn and non-syn sites according to \emph{Nei and Gojobori (1986)}.
...
compareCodons <- function(codA, codB){
  ...
  if(length(codAcodB_diff_idx) == 2){
    ...
    tmp_syn <- 0
    if(GENETIC_CODE_TCAG[codA, 2] == GENETIC_CODE_TCAG[codC, 2]){
      tmp_syn <- tmp_syn + 1
    }
    if(GENETIC_CODE_TCAG[codA, 2] == GENETIC_CODE_TCAG[codD, 2]){
      tmp_syn <- tmp_syn + 1
    }
    if(GENETIC_CODE_TCAG[codB, 2] == GENETIC_CODE_TCAG[codC, 2]){
      tmp_syn <- tmp_syn + 1
    }
    if(GENETIC_CODE_TCAG[codB, 2] == GENETIC_CODE_TCAG[codD, 2]){
      tmp_syn <- tmp_syn + 1
    }
    tmp_syn <- tmp_syn / 2
  ...
  if(length(codAcodB_diff_idx) == 3){
    ...
    if(GENETIC_CODE_TCAG[codA, 2] == GENETIC_CODE_TCAG[codE, 2]){
      tmp_syn <- tmp_syn + 1
    }
    if(GENETIC_CODE_TCAG[codB, 2] == GENETIC_CODE_TCAG[codF, 2]){
      tmp_syn <- tmp_syn + 1
    }
    if(GENETIC_CODE_TCAG[codB, 2] == GENETIC_CODE_TCAG[codG, 2]){
      tmp_syn <- tmp_syn + 1
    }
    if(GENETIC_CODE_TCAG[codB, 2] == GENETIC_CODE_TCAG[codH, 2]){
      tmp_syn <- tmp_syn + 1
    }
    if(GENETIC_CODE_TCAG[codC, 2] == GENETIC_CODE_TCAG[codF, 2]){
      tmp_syn <- tmp_syn + 0.5
    }
    if(GENETIC_CODE_TCAG[codC, 2] == GENETIC_CODE_TCAG[codG, 2]){
      tmp_syn <- tmp_syn + 0.5
    }
    if(GENETIC_CODE_TCAG[codD, 2] == GENETIC_CODE_TCAG[codF, 2]){
      tmp_syn <- tmp_syn + 0.5
    }
    if(GENETIC_CODE_TCAG[codD, 2] == GENETIC_CODE_TCAG[codH, 2]){
      tmp_syn <- tmp_syn + 0.5
    }
    if(GENETIC_CODE_TCAG[codE, 2] == GENETIC_CODE_TCAG[codG, 2]){
      tmp_syn <- tmp_syn + 0.5
    }
    if(GENETIC_CODE_TCAG[codE, 2] == GENETIC_CODE_TCAG[codH, 2]){
      tmp_syn <- tmp_syn + 0.5
    }
    tmp_syn <- tmp_syn / 3
